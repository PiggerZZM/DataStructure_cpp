...
public:
    ...
    bool getData(int i, Type &x) const; // 取下标为i的元素的值，用x返回
    bool setData(int i, Type x) const;  // 将下标为i的元素的值设为x
...
template <class Type>
bool SeqList<Type>::getData(int i, Type &x) const // 取下标为i的元素的值，用x返回
{
    if (i >= 0 && i <= last)
    {
        x = data[i];
